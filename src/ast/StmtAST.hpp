...
// ParallelForStmtAST
//
// Data-parallel iteration — every element processed independently and
// simultaneously. Primary DOD (data-oriented design) form.
//
...
//   - no writes to variables declared outside the parallel scope
...
struct ParallelForStmtAST : StmtAST {
    ...
    ParallelForStmtAST() : StmtAST(ASTKind::ParallelForStmt) {}
...
//
// Task-parallel execution — a fixed set of independent blocks run
// simultaneously. All complete before execution continues past the parallel
// block. Analogous to Promise.all or Go goroutines.
//
//   parallel {
...
//   }
//   -- all three complete before this line is reached
//
// subBlocks — one BlockStmtAST per independent task, in source order.
//   Minimum one sub-block (a parallel block with zero tasks is a parse error).
//
// Semantic pass enforces inside every sub-block (sets isParallel on each):
//   - no writes to variables declared outside the parallel scope
//   - no await expressions
//   - no return statements
//   - no break or continue statements
