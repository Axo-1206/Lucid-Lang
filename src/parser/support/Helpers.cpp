...
// =============================================================================
// parseFuncTypeParts — the shared stage reader
// =============================================================================

/// @brief Read the stages of a function type or declaration header.
///
/// Grammar (shared by both call sites):
///   parts = stage { stage | '->' stage } [ '->' type ]
///   stage = ( 'fn' | 'cls' ) group
///
/// Adjacency and arrow-separation are equivalent; both just mean "another
/// stage follows." The marker is mandatory on every stage, so a bare
/// `(a int)(b int)` after the first stage is a parse error.
///
/// This function does NOT build any AST chain. It only reads the stages and
/// the final return type. The caller decides what to build from them —
/// a FuncTypeAST chain for a type, or an AnonFuncExprAST chain for a
/// declaration body.
///
/// @param stream     The token stream, positioned at the first marker.
/// @param ctx        The parsing context.
/// @param allowNames Whether parameter names are permitted. `true` for
///                   declarations and anonymous functions, `false` for
///                   types.
/// @return The parsed stages and the final return type (nullptr for void).
///         On error, still returns whatever stages were successfully read,
///         with an `UnknownTypeAST` as the final return type if the return
///         type failed to parse.
FuncTypeParts parseFuncTypeParts(TokenStream& stream, ParserContext& ctx,
                                 bool allowNames) {
    FuncTypeParts parts;

    // ─── 1. Read stages until we hit a final return type or the end ────────
    while (true) {
        // ─── 1a. Marker error ──────────────────────────────────────────────────
        if (!is_function_type_keyword(stream.peekType())) {
            ctx.diagnostics.errorAt(
                DiagCode::Syntax_ExpectedToken,
                stream.currentLoc(),
                "expected 'fn' or 'cls' before parameter group, got '",
                stream.peekValue(), "'");
            // Empty stage so downstream has something to chain. The default-
            // constructed ArenaSpan<ParamAST*> is the canonical empty span:
            // size 0, data nullptr, valid for iteration.
            parts.stages.push_back({FuncShape::Fn, {}, stream.currentLoc()});
            return parts;
        }
        ...
        FuncShape shape = (markerTok.type == TokenType::TYPE_FN)
                          ? FuncShape::Fn
                          : FuncShape::Cls;
        SourceLocation stageLoc = SourceLocation(markerTok.line, markerTok.column);
        ...
        if (!stream.check(TokenType::LPAREN)) {
            ...
            parts.stages.push_back({shape, {}, stageLoc});
            return parts;
        }

        std::vector<ParamAST*> groupParams = parseParamList(stream, ctx, allowNames);
        auto paramBuilder = ctx.arena.makeBuilder<ParamAST*>();
        for (ParamAST* p : groupParams) {
            paramBuilder.push_back(p);
        }
        parts.stages.push_back({shape, paramBuilder.build(), stageLoc});

        // ─── 1c. What follows? ─────────────────────────────────────────────
        //   - another marker → adjacent stage, loop
        //   - '->' + marker  → arrow-separated stage, loop
        //   - '->' + type    → final return type, stop
        //   - anything else  → void return, stop
        if (is_function_type_keyword(stream.peekType())) {
            continue;   // adjacent form
        }
        ...
        if (stream.check(TokenType::ARROW)) {
            ...
            parts.finalReturnType = ret;
            return parts;
        }

        // No arrow, no marker: void return.
        parts.finalReturnType = nullptr;
        return parts;
    }
}

// =============================================================================
// buildFuncTypeChain — stages → right-nested FuncTypeAST chain
// =============================================================================

/// @brief Build a right-nested FuncTypeAST chain from parsed stages.
///
/// Given stages [S0, S1, ..., Sn] and a final return type R, produces:
///
///   S0(returnType = S1(returnType = ... Sn(returnType = R)))
///
/// Each nested node carries its own stage's `shape` and `params`.
FuncTypeAST* buildFuncTypeChain(ParserContext& ctx, const FuncTypeParts& parts) {
    if (parts.stages.empty()) {
        auto* ft = ctx.arena.make<FuncTypeAST>();
        ft->params = {};
        ft->shape = FuncShape::Fn;
        ft->returnType = parts.finalReturnType;
        ft->hasSyntaxError = true;
        return ft;
    }

    TypeAST* currentReturnType = parts.finalReturnType;

    for (int i = static_cast<int>(parts.stages.size()) - 1; i >= 0; --i) {
        auto* ft = ctx.arena.make<FuncTypeAST>();
        ft->params = parts.stages[i].params;
        ft->shape = parts.stages[i].shape;
        ft->returnType = currentReturnType;
        ft->loc = parts.stages[i].loc;
        currentReturnType = ft;
    }

    return static_cast<FuncTypeAST*>(currentReturnType);
}

// =============================================================================
// buildAnonFuncChain — stages + body → right-nested AnonFuncExprAST chain
// =============================================================================

/// @brief Build a right-nested AnonFuncExprAST chain from parsed stages.
///
/// Given stages [S0, S1, ..., Sn], a final return type R, and a body B:
///
///   - The innermost stage Sn becomes an AnonFuncExprAST whose funcType is
///     `Sn -> R` and whose body is B.
///   - Each outer stage Si becomes an AnonFuncExprAST whose funcType is
///     `Si -> <inner stage type>` and whose body is `{ return <inner anon>; }`.
///   - The returned node is the outermost anon (S0's).
///
/// This is the curried desugaring described in the design plan: partial
/// application falls out of the fact that each outer anon captures the
/// parameters of the inner ones.
///
/// The caller supplies the body already wrapped as a StmtAST. This function
/// does not parse anything; it only builds the AST chain.
AnonFuncExprAST* buildAnonFuncChain(ParserContext& ctx,
                                    const FuncTypeParts& parts,
                                    StmtAST* body) {
    if (parts.stages.empty() || !body) {
        // Error path: fabricate a single empty anon.
        auto* ft = ctx.arena.make<FuncTypeAST>();
        ft->params = {};
        ft->shape = FuncShape::Fn;
        ft->returnType = parts.finalReturnType;
    ...
    // ─── Build the FuncTypeAST chain once, so we can reuse each stage's type ─
    FuncTypeAST* outerType = buildFuncTypeChain(ctx, parts);

    // Walk down the chain to collect each stage's type in order.
    std::vector<FuncTypeAST*> stageTypes;
    stageTypes.reserve(parts.stages.size());
    for (FuncTypeAST* ft = outerType; ft != nullptr; ft = ft->getNext()) {
        stageTypes.push_back(ft);
    }
    // stageTypes[i] corresponds to parts.stages[i].

    // ─── Build the anon chain from the inside out ──────────────────────────
    ...
    for (int i = static_cast<int>(parts.stages.size()) - 1; i >= 0; --i) {
        ...
        currentAnon->loc = parts.stages[i].loc;

        if (i == 0) break;

        // Wrap in `return <inner>;` for the next outer anon's body.
        auto* ret = ctx.arena.make<ReturnStmtAST>();
        ret->loc = currentAnon->loc;
