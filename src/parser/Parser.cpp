 ...
 *    d. ScopedFileContext   — save this file's caller's contextStack, reset
 *                             it for this file (this file starts at
 *                             TopLevel), and open a fresh diagnostic scope
 *                             for this file via an embedded
 *                             diagnostic::ScopedSource; restore the
 *                             caller's contextStack — and let the caller's
 *                             diagnostic scope reappear underneath — on
 *                             return
 ...
 *    i. Build ModuleAST from that decl list; hasErrors =
 *       diagnostic::hasErrorsInCurrentSource() (this file's own diagnostics
 *       remain retrievable afterward via diagnostic::getAllForFile(path) —
 *       see BaseAST.hpp's ModuleAST doc comment)
 ...
 *       contextStack, close this file's diagnostic scope]
 *    l. return thisModule
 *
 ...
 * |-----------------------|--------------------------------------------------|------------------------------|-------------------------|-----------------------------------------------------------------------|
 * | `ScopedContext`       | one `ContextFrame` (push one, pop one)          | `ParserContext::contextStack` | many — one per attribute list, generic-arg list, function/struct/enum/trait body, etc. | "What syntactic construct is the parser inside *right now*, within this file?" |
 * | `ScopedFileContext`   | `contextStack` (save → reset → restore) directly, plus this file's diagnostic tracking via an embedded `diagnostic::ScopedSource` (push → ... → pop) | `ParserContext` and (via the embedded member) `diagnostic::`'s source-scope stack | exactly one — the whole file is one activation | "Whose file's syntax-nesting state, and which file's diagnostic scope, is currently live?" |
 * | `ScopedParsingGuard`  | one entry in `parsingStack_` (push one path, pop one path) | `ModuleResolver::parsingStack_` | exactly one — the whole file is one activation | "Which file paths are currently mid-parse, up the *entire* call stack (including files that imported this one)?" |
 *
...
ModuleAST* parse(const std::string& path, 
    ...
    // Save the importing file's contextStack (if any), reset it for this
    // file, and restore it on return — on every exit path below, including
    // the early returns. Also opens this file's own diagnostic scope (via
    // an embedded diagnostic::ScopedSource), so every ctx.error()/warning()
    // call below is automatically attributed to this file with no `file`
    // argument needed at the call site — see ScopedFileContext's own doc
    // comment.
    ...
    for (const auto& tok : tokens) {
        if (tok.type == TokenType::UNKNOWN) {
            ...
            auto* mod = ctx.arena.make<ModuleAST>();
            mod->filePath = filePath;
            mod->hasErrors = true;   // known true here regardless — the E0105
                                      // just reported above guarantees it; no
                                      // need to ask diagnostic:: for what we
                                      // already know.
            if (ctx.resolver) ctx.resolver->cacheModule(filePath, mod);
            return mod;
        }
    ...
    // This module's own diagnostics are retrievable at any time afterward
    // via diagnostic::getAllForFile(thisModule->filePath) — see BaseAST.hpp's
    // ModuleAST doc comment for why the node itself no longer stores them.
    // Safe to read hasErrorsInCurrentSource() here: this file's
    // diagnostic::ScopedSource (opened earlier via `fileContext`, step 2d
    // above) is still the innermost scope at this point in the function.
 ...
 *     │                     real ModuleAST and flags it via diagnostic::hasErrorsInCurrentSource())
 *     │
 *     └── 4. Return
 ...
 * // decls contains everything collected, whether or not diagnostic::hasErrorsInCurrentSource() —
 ...
 *       through. Check diagnostic::hasErrorsInCurrentSource() (or the resulting ModuleAST::hasErrors)
...
void parseInternal(TokenStream& stream, ParserContext& ctx, std::vector<DeclPtr>& outDecls) {
    ...
    while (!stream.isAtEnd() && consecutiveFailures < MAX_CONSECUTIVE_FAILURES) {
        ...
        if (!decl && stream.isAtEnd()) {
            ...
            if (diagnostic::hasErrorsInCurrentSource()) {
                break;
            }
    ...
    if (stream.isAtEnd() && diagnostic::hasErrorsInCurrentSource()) {
    ...
    if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
        ...
        // ModuleAST from it and flags hasErrors via diagnostic::hasErrorsInCurrentSource(), rather
