...
// ─────────────────────────────────────────────────────────────────────────────
// visit(ExtensionDeclAST)
//
// Registers extension methods in the symbol table.
// Extension methods are static functions stored under mangled names:
//   - Format: "Type::namespace.method"
//   - Example: "int::std.abs"
//
// The symbol kind is SymbolKind::Func (or ExternFunc if @extern is present).
// The visibility of the extension block applies to all methods inside.
// ─────────────────────────────────────────────────────────────────────────────
void SemanticCollector::visit(ExtensionDeclAST& node) {
    LUC_LOG_SEMANTIC_VERBOSE("SemanticCollector: visiting ExtensionDeclAST for type: " 
                             << (node.targetType ? "some type" : "null")
                             << ", namespace: " << getNameString(node.namespaceName));

    // Extract the type name string for mangling
    std::string_view typeNameStr;
    
    if (node.targetType) {
        // Handle primitive types (int, float, string, etc.)
        if (node.targetType->isa<PrimitiveTypeAST>()) {
            auto* prim = node.targetType->as<PrimitiveTypeAST>();
            switch (prim->primitiveKind) {
                case PrimitiveKind::Bool:   typeNameStr = "bool"; break;
                case PrimitiveKind::Byte:   typeNameStr = "byte"; break;
                case PrimitiveKind::Short:  typeNameStr = "short"; break;
                case PrimitiveKind::Int:    typeNameStr = "int"; break;
                case PrimitiveKind::Long:   typeNameStr = "long"; break;
                case PrimitiveKind::Ubyte:  typeNameStr = "ubyte"; break;
                case PrimitiveKind::Ushort: typeNameStr = "ushort"; break;
                case PrimitiveKind::Uint:   typeNameStr = "uint"; break;
                case PrimitiveKind::Ulong:  typeNameStr = "ulong"; break;
                case PrimitiveKind::Int8:   typeNameStr = "int8"; break;
                case PrimitiveKind::Int16:  typeNameStr = "int16"; break;
                case PrimitiveKind::Int32:  typeNameStr = "int32"; break;
                case PrimitiveKind::Int64:  typeNameStr = "int64"; break;
                case PrimitiveKind::Uint8:  typeNameStr = "uint8"; break;
                case PrimitiveKind::Uint16: typeNameStr = "uint16"; break;
                case PrimitiveKind::Uint32: typeNameStr = "uint32"; break;
                case PrimitiveKind::Uint64: typeNameStr = "uint64"; break;
                case PrimitiveKind::Float:  typeNameStr = "float"; break;
                case PrimitiveKind::Double: typeNameStr = "double"; break;
                case PrimitiveKind::Decimal:typeNameStr = "decimal"; break;
                case PrimitiveKind::String: typeNameStr = "string"; break;
                case PrimitiveKind::Char:   typeNameStr = "char"; break;
                case PrimitiveKind::Any:    typeNameStr = "any"; break;
            }
        }
        // Handle named types (structs, enums, type aliases)
        else if (node.targetType->isa<NamedTypeAST>()) {
            auto* named = node.targetType->as<NamedTypeAST>();
            typeNameStr = getNameString(named->name);
        }
    }

    if (typeNameStr.empty()) {
        LUC_LOG_SEMANTIC("SemanticCollector: cannot resolve target type for extension block");
        _dc.error(DiagnosticCategory::Semantic, node.loc, DiagCode::E3001,
                  "cannot resolve target type for extension block");
        return;
    }

    // Register each method in the extension block
    for (auto& method : node.methods) {
        if (!method) continue;

        std::string_view methodName = getNameString(method->name);
        
        // Mangle the name: "Type::namespace.method"
        std::string mangledName = NameMangler::mangleExtension(
            typeNameStr,
            getNameString(node.namespaceName),
            methodName
        );

        LUC_LOG_SEMANTIC_EXTREME("\tRegistering extension method: " << mangledName);

        // Create the symbol
        Symbol sym;
        sym.name = _pool.intern(mangledName);
        sym.kind = SymbolKind::Func;
        sym.declKw = method->keyword;
        sym.visibility = node.visibility;
        sym.type = nullptr;  // Will be set by TypeResolver
        sym.decl = method.get();
        sym.loc = method->loc;

        // Extract @extern metadata if present
        extractExternMetadata(method->attributes, sym);
