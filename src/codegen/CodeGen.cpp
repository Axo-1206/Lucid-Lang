...
CodegenResult generate(const std::vector<ModuleAST*>& modules,
    ...
    if (!verified) {
        ...
        // The manifest is populated but the module is invalid. Clear it
        // so callers can't accidentally use it.
    ...
    // ─── 3. Move the module and context out ──────────────────────────────
    result.llvmCtx = program.releaseContext();
    result.module = program.releaseModule();
    result.success = true;
