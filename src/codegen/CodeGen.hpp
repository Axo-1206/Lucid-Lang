...
/// @brief Load a value from a pointer if needed.
///
/// @param value The value or pointer.
/// @param isLValue Whether the value is an l-value (should be loaded).
/// @param ctx The code generation context.
/// @return The loaded value if l-value, otherwise the original value.
llvm::Value* loadIfNeeded(llvm::Value* value, bool isLValue, CodeGenContext& ctx);

/// @brief Get the LLVM type for a Lucid type (convenience wrapper).
///
/// @param ctx The code generation context.
/// @param type The Lucid type.
/// @return The LLVM type.
inline llvm::Type* getLLVMType(CodeGenContext& ctx, const TypeAST* type) {
    return getType(ctx, type);
}

/// @brief Create an alloca for a local variable.
///
...
/// @brief Create a basic block with a name.
///
